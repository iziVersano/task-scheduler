...
```mermaid
flowchart LR
    A["Select policy type\n(S3 Bucket Policy)"] --> B["Set Effect\nAllow / Deny"]
    B --> C["Set Principal\n* = everyone\nor specific IAM ARN"]
    C --> D["Set Actions\ne.g. s3:DeleteObject\ns3:PutObject"]
    D --> E["Set Resource\nBucket ARN + /*"]
    E --> F["Generate Policy JSON\ncopy → paste into bucket"]
```

> 🏢 **Real world:** A startup onboarding junior developers needs to prevent accidental deletion of production data in S3. Instead of writing IAM JSON from scratch (and risking a typo that grants too much access), their DevOps lead uses the Policy Generator: selects S3 Bucket Policy, sets Effect = Deny, Action = `s3:DeleteObject`, Principal = `*`, Resource = the production bucket ARN. Generated in 30 seconds, zero JSON errors, pasted directly into the bucket — junior devs can upload but can never delete.
